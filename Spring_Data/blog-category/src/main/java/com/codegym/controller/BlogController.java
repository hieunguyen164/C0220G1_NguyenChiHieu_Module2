...
public class BlogController {
    ...
    @ModelAttribute("categorize")
    public Iterable<Category> categorize(){
    ...
    public ModelAndView listBlogs(@RequestParam("search") Optional<String> search, Pageable  pageable){
        Page<Blog> blogs;
        Pageable pageableSortByBlogTitle = PageRequest.of(pageable.getPageNumber(),3,Sort.by("title").ascending());
        if(search.isPresent()){
            blogs = blogService.findAllByTitleContaining(search.get(),pageableSortByBlogTitle);
        }else{
            blogs = blogService.findAll(pageableSortByBlogTitle);
        }
